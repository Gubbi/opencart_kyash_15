function preparePullShops(url) {
    ...
    var kyash_code = document.getElementById("kyash_code").value;
    pullNearByShops(postcode, url, kyash_code);
}
var old_postcode = "";
var errorMessage = "Due to some unexpected errors, this is not available at the moment. We are working on fixing it.";
function pullNearByShops(postcode, url, kyashcode) {
    if (postcode.length == 0) {
        ...
        if (old_postcode != postcode) {
            ...
            $.ajax({
                url: url + "&postcode=" + postcode + "&kyashcode=" + kyashcode,
